# PostgreSQL schema gap analysis

Audit date: 2026-07-13. The authoritative runtime model is the 26 tables in
...
| JSON | candidates use `JSONB`; app passes and parses canonical JSON strings | Baseline stores JSON as text so SQLite/PostgreSQL API values remain identical |
...
| Boolean flags | candidates use `BOOLEAN`; application SQL and fixtures use `0`/`1` | Baseline uses checked integers for exact cross-backend SQL behavior |
| Floating point | SQLite `REAL` is 64-bit; PostgreSQL `REAL` is 32-bit | Baseline maps it to `DOUBLE PRECISION` so coordinates and JSON responses remain exact |
